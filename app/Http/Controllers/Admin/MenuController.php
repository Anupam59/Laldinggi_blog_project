...
class MenuController extends Controller
{
    ...
    public function MenuSubSubItemIndex(){

        $MenuSubSubItem = MenuSubSubItemModel::join('users as creator_by', 'creator_by.id', '=', 'menu_sub_sub_item.creator')
            ->leftJoin('users as modifier_by', 'modifier_by.id', '=', 'menu_sub_sub_item.modifier')
            ->leftJoin('menu', 'menu.menu_id', '=', 'menu_sub_sub_item.menu_id')
            ->leftJoin('menu_item', 'menu_item.menu_item_id', '=', 'menu_sub_sub_item.menu_item_id')
            ->leftJoin('menu_sub_item', 'menu_sub_item.menu_sub_item_id', '=', 'menu_sub_sub_item.menu_sub_item_id')
            ->select(
                'creator_by.name as creator_by',
                'modifier_by.name as modifier_by',
                'menu.menu_title',
                'menu_item.menu_item_title',
                'menu_sub_item.menu_sub_item_title',
                'menu_sub_sub_item.*'
            )
            ->orderBy('menu_sub_sub_item_id','asc')->paginate(10);

        return view('Admin/Pages/Menu/MenuSubSubItemIndex',compact('MenuSubSubItem'));
    }
    public function MenuSubSubItemCreate(){
        $Menu = MenuModel::where('status',1)->select('menu_id','menu_title')->get();
        return view('Admin/Pages/Menu/MenuSubSubItemCreate',compact('Menu'));
    }
    public function MenuSubSubItemEntry(Request $request){
        $validation = $request->validate([
            'menu_id' => 'required',
            'menu_item_id' => 'required',
            'menu_sub_item_id' => 'required',
            'menu_sub_sub_item_title' => 'required',
            'menu_sub_sub_item_bn_title' => 'required',
            'menu_sub_sub_item_link' => 'required',
        ]);
        $data =  array();
        $data['menu_id'] = $request->menu_id;
        $data['menu_item_id'] = $request->menu_item_id;
        $data['menu_sub_item_id'] = $request->menu_sub_item_id;
        $data['menu_sub_sub_item_title'] = $request->menu_sub_sub_item_title;
        $data['menu_sub_sub_item_bn_title'] = $request->menu_sub_sub_item_bn_title;
        $data['menu_sub_sub_item_link'] = $request->menu_sub_sub_item_link;
        ...
        $res = MenuSubSubItemModel::insert($data);

        if ($res){
            return back()->with('success_message','Menu Sub Sub Item Add Successfully!');
        }else{
            return back()->with('error_message','Menu Sub Sub Item Add Fail!');
        }
    }
    public function MenuSubSubItemEdit($id){
        $Menu = MenuModel::where('status',1)->get();
        $MenuSubSubItem = MenuSubSubItemModel::where('menu_sub_sub_item_id',$id)->first();
        $menu_id = $MenuSubSubItem->menu_id;
        $menu_item_id = $MenuSubSubItem->menu_item_id;
        $MenuItem = MenuItemModel::where('menu_id',$menu_id)->where('status',1)->get();
        $MenuSubItem = MenuSubItemModel::where('menu_item_id',$menu_item_id)->where('status',1)->get();

        return view('Admin/Pages/Menu/MenuSubSubItemUpdate',compact('Menu','MenuSubSubItem','MenuItem','MenuSubItem'));
    }
    public function MenuSubSubItemUpdate(Request $request, $id){
        ...
        $res = MenuSubSubItemModel::where('menu_sub_sub_item_id','=',$id)->update($data);

        if ($res){
            return back()->with('success_message','Menu Sub Sub Item Update Successfully!');
        }else{
            return back()->with('error_message','Menu Sub Sub Item Update Fail!');
        }
